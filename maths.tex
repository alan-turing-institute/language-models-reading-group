...
Suppose $\mathcal{A}_X$ and $\mathcal{A}_Y$ are sets, and suppose $J = (\mathcal{A}_X\times\mathcal{A}_Y, p_J)$ is a joint ensemble. Consider the probability mass function $p_X$ on $\mathcal{A}_X$ given by ``summing over the y's'', that is $p_X(x) \isdef \sum_{y\in\mathcal{A}_Y} p_J(x,y)$. We obtain an ensemble $(\mathcal{A}_X, p_X)$ called the \defn{marginal} distribution on~$\mathcal{A}_X$. Likewise, we may obtain the marginal distribution on~$\mathcal{A}_Y$ given by ``summing over the x's'', $p_Y \isdef \sum_{x\in\mathcal{A}_X} p_J(x,y)$.

We often abuse notation and write $P(x,y)$ for the joint probability of the event $\{(x,y)\}$ represending the single outcome $(x,y)\in\mathcal{A}_X\times\mathcal{A}_Y$. Likewise we write $P(x)$ for the probability in the marginal distribution of the event $\{x\}$.

The probability mass function on $\mathcal{A}_X$ given by $p_C \isdef = p_J(x,y)/p_Y(y)$ is called the \defn{conditional distribution}.  We write $P(x\mid y)$ for the probability 
